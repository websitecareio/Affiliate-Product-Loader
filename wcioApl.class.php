<?php
if (!class_exists("wcio_affiliate_product_listing")) {
      class wcio_affiliate_product_listing {
            ...
            function wcio_apl_remove_products( $feedId, $prefix = "" ) {

                  $table = $prefix."wcio_apl_products";
                  $stmt = $this->pdo->prepare("DELETE FROM $table WHERE feedId = :feedId");
                  $stmt->execute(array(
                        ":feedId" => (int)$feedId
                  ));
            ...
            function wcio_apl_save_product( $feedId, $feedUrl, $productData, $prefix = "" ) {
                        ...
                        // If there no productSku then set the sku to productid
                        if($productSku == "") {
                              $productSku = $productData->produktid;
                        }
                        if($productPriceOld == "") {
                              $productPriceOld = $productData->glpris;
                        }
                        ...
                        $stmt->execute(array(
                              ...
                              ":stockLevel" => (int)$productData->lagerantal,
                              ...
                              ":productPriceOld" => number_format((float)$productPriceOld, 2, '.', '')*100,
            ...
            function wcio_apl_update_feed( $feedId = NULL, $prefix = "" ) {

                  $table = $prefix."wcio_apl_feeds";
                  if( $feedId ) {

                        // Fetch feed from DB and update the prodcuts table
                        $stmt = $this->pdo->prepare("SELECT id,feedUrl FROM $table WHERE id = :id");
                        $stmt->execute(array(
                              ":id" => $feedId,
                        ));
                        ...
                        $stmt = $this->pdo->prepare("SELECT id,feedUrl FROM $table ORDER BY id");
                        $stmt->execute();
                        $getData = $stmt->fetchAll();
                  ...
                  foreach( $getData as $data ) {
                        ...
                        $this->wcio_apl_remove_products( $feedId, $prefix );
/*
...
*/
                        $xmlResponse = simplexml_load_file($feedUrl);

                        // Loop the products and inset into table
                        foreach( $xmlResponse AS $productData ) {

                              // Save the product to database
                              $this->wcio_apl_save_product($feedId, $feedUrl, $productData, $prefix);

                        }

                        // Update the field "updated" to match current time
                        $stmt = $this->pdo->prepare("UPDATE $table SET updated = :updated WHERE id = :id");
                        $stmt->execute(array(
                              ":updated" => time(),
                              ":id" => $feedId
                        ));
            ...
            function wcio_apl_add_feed( $feedUrl, $feedType, $prefix = ""  ) {

                  $table = $prefix."wcio_apl_feeds";

                  // Update the field "updated" to match current time
                  $stmt = $this->pdo->prepare("INSERT INTO $table (feedUrl, feedType) VALUES (:feedUrl, :feedType)");
                  $stmt->execute(array(
                        ":feedUrl" => $feedUrl,
                        ":feedType" => $feedType
                  ));
            ...
            function wcio_apl_delete_feed( $feedId, $prefix = ""  ) {
            ...
            function wcio_apl_get_products( $searchColumn, $searchColumnType, $searchColumnValue, $orderBy, $orderByType, $secondaryOperator = " AND ", $secondarySearch, $limitTo = "100", $startFrom = "0", $prefix) {
      ...
      */
                  ...
                  // Validate inputs
                  $searchColumn = $this->wcio_apl_whitelist( $searchColumn, ["id","productDealer","category","brand","productEan","productSku","productName","productPrice","productDiscount","productId"], "productName");
                  ...
                  $table = $prefix."wcio_apl_products";
                  $stmt = $this->pdo->prepare("SELECT * FROM $table WHERE $searchColumn $searchColumnType lower(:searchColumnValue) $secondarySearchOutput ORDER BY $orderBy $orderByType LIMIT $startFrom, $limitTo");
                  $stmt->execute(array(
                        "searchColumnValue" => strtolower($searchColumnValue),
                  ));
                  $getData = $stmt->fetchAll();
